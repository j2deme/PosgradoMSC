...
$app -> get('/admin/', function() use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"));
    $data['user'] = isAllowed("Administrador", false);
    $app -> render('dashboard.html', $data);
}) -> name('admin');

$app -> get('/admin/usuarios/', function() use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"), array("name" => "Gestor de Usuarios", "alias" => "admin-usuarios"));
    $data['user'] = isAllowed("Administrador", false);
...
$app -> post('/nuevo-usuario/', function() use ($app) {
    $validator = new GUMP();
    $_POST = $validator -> sanitize($_POST);
    $rules = array('usuario' => 'required|alpha_dash|max_len,100', 'email' => 'required|valid_email', );

    $filters = array('usuario' => 'trim|sanitize_string', 'email' => 'trim|sanitize_email');
    ...
    if ($validated === true) {
        ...
        if ($ok) {
            ...
            $mailData = array("to" => $_POST['email'], "subject" => "Activación de cuenta", "body" => "", "from" => "posgradomsc@gmail.com", "alias" => "Posgrado MSC IT Cd. Victoria");
            ...
            $flash = array("title" => "ERROR", "msg" => "Ha sucedido un error inesperado, intente nuevamente.", "type" => "error", "fade" => 0);
        }
        $app -> flash("flash", $flash);
        $app -> flashKeep();
...
$app -> post('/actualiza-usuario/:id/', function($id) use ($app) {
    ...
    $rules = array('usuario-edit' => 'required|alpha_dash|max_len,100', 'email-edit' => 'required|valid_email', );

    $filters = array('usuario-edit' => 'trim|sanitize_string', 'email-edit' => 'trim|sanitize_email', 'password' => 'trim|md5');
    $post = $_POST = $validator -> filter($_POST, $filters);
    $validated = $validator -> validate($_POST, $rules);
    if ($validated === true) {
        ...
        if ($ok) {
            ...
            $flash = array("title" => "OK", "msg" => "El usuario se ha actualizado con éxito.", "type" => "success", "fade" => 1);
        } else {
            $flash = array("title" => "ERROR", "msg" => "Ha sucedido un error inesperado, intente nuevamente.", "type" => "error", "fade" => 0);
        }
        $app -> flash("flash", $flash);
...
$app -> post('/actualiza-usuario/', function() use ($app) {
    $validator = new GUMP();
    $_POST = $validator -> sanitize($_POST);
    $rules = array('confirmacion' => 'required', 'login' => 'required', 'paspassword' => 'required', 'password' => 'required', 'id' => 'required', );

    $filters = array('paspassword' => 'trim|md5', 'password' => 'trim|md5', 'confirmacion' => 'trim|md5');
    $post = $_POST = $validator -> filter($_POST, $filters);
    $validated = $validator -> validate($_POST, $rules);
    if ($validated === true) {
        ...
        if ($u -> password == $_POST['paspassword']) {
            if ($_POST['password'] == $_POST['confirmacion']) {
                ...
                $flash = array("title" => "ERROR", "msg" => "El campo de contraseña nuevo y confirmacion debe ser iguales.", "type" => "error", "fade" => 0);

            }
        } else {

            $flash = array("title" => "ERROR", "msg" => "La contraseña actual es incorrecta.", "type" => "error", "fade" => 0);
...
$app -> get('/cambiar-status-usuario/:id/', function($id) use ($app) {
    ...
    $flash = array("title" => "OK", "msg" => "El usuario ha sido $verb correctamente.", "type" => "info", "fade" => 1);
    $app -> flash("flash", $flash);
    $app -> flashKeep();
    $app -> redirect($app -> urlFor('admin-usuarios'));
...
$app -> get('/admin/roles/', function() use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"), array("name" => "Gestor de Roles", "alias" => "admin-roles"));
    $data['user'] = isAllowed("Administrador", false);
...
$app -> get('/admin/aspirantes/', function() use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"), array("name" => "Gestor de Aspirantes", "alias" => "admin-aspirantes"));
    $data['user'] = isAllowed("Administrador", false);
...
$app -> get('/procesar-aspirante/:action/:id/', function($action, $id) use ($app) {
    ...
    $ur -> save();
    $flash = array("title" => "OK", "msg" => "Aspirante $verb.", "type" => "info", "fade" => 1);
    $app -> flash("flash", $flash);
    $app -> flashKeep();
...
$app -> get('/admin/archivos/', function() use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"), array("name" => "Gestor de Archivos", "alias" => "admin-archivos"));
    $data['user'] = isAllowed("Administrador", false);
...
$app -> map("/admin/elFinder/", function() use ($app) {
    $opts = array(
    // 'debug' => true,
    'roots' => array( array('driver' => 'LocalFileSystem', // driver for accessing file system (REQUIRED)
...
$app -> get('/download/:file', function($file) use ($app) {
    ...
    $file = __DIR__ . "/" . $file;
    if (!file_exists($file)) {
        $flash = array("title" => "ERROR", "msg" => "No se encontro el archivo $file", "type" => "error", "fade" => 1);
        $app -> flash("flash", $flash);
        $app -> flashKeep();
...
$app -> get('/admin/secciones/', function() use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"), array("name" => "Gestor de Secciones", "alias" => "admin-secciones"));
    $data['user'] = isAllowed("Administrador", false);
...
$app -> get('/admin/secciones/editor/:slug', function($slug) use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"), array("name" => "Gestor de Secciones", "alias" => "admin-secciones"), array("name" => "Editor", "alias" => "editor-seccion"));
...
$app -> post('/actualiza-seccion/:id/', function($id) use ($app) {
    ...
    if ($validated === true) {
        ...
        $seccion -> save();
        $flash = array("title" => "OK", "msg" => "La seccion se ha actualizado con éxito.", "type" => "success", "fade" => 1);
        $app -> flash("flash", $flash);
    } else {
        $msgs = humanize_gump($validated);
        $flash = array("title" => "ERROR", "msg" => $msgs, "type" => "error", "fade" => 0);
...
$app -> get('/admin/noticias/', function() use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"), array("name" => "Gestor de Noticias", "alias" => "admin-noticias"));
    $data['user'] = isAllowed("Administrador", false);
}) -> name('admin-noticias');

$app -> get('/admin/catalogos/', function() use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"), array("name" => "Catálogos", "alias" => "admin-catalogos"));
    $data['user'] = isAllowed("Administrador", false);
...
$app -> get('/estadisticas/matriculacion/', function() use ($app) {
    ...
    $data['matriculaciones']= Matriculacion::all(array('order'=>'periodo'));
...
$app -> get('/docente/publicaciones-post', function() use ($app) {
    ...
    $_POST = $validator -> sanitize($_POST);
    if ($_POST['tipo'] == 1) {
        $rules = array('mautor' => 'required|alpha', 'mtitulo' => 'required|alpha', 'mpublicacion' => 'required|alpha', 'mfechapublicacion' => 'required', );
        $filters = array();
        $_POST = $validator -> filter($_POST, $filters);
        $validated = $validator -> validate($_POST, $rules);
        if ($validated === TRUE) {
            ...
            $flash = array("title" => "OK", "msg" => "ha ocurrido un problema. Vuelva a intenrar.", "type" => "error", "fade" => 1);
            $app -> flash("flash", $flash);
            $app -> flashKeep();
            $app -> redirect($app -> urlFor('docente-publicaciones'));
        }
        ...
        if ($_POST['tipo'] == 2) {
            $rules = array('rautor' => 'required|alpha', 'rtitulo' => 'required|alpha', 'revista' => 'required|alpha', 'rtipo' => 'required', 'rfechapublicacion' => 'required');
            $filters = array();
            $_POST = $validator -> filter($_POST, $filters);
            $validated = $validator -> validate($_POST, $rules);
            if ($validated === TRUE) {
            ...
            if ($_POST['tipo'] == 3) {
                $rules = array('lautor' => 'required|alpha', 'ltitulo' => 'required|alpha', 'editorial' => 'required|alpha', 'isbn' => 'required|numeric', 'lfechapublicacion' => 'required', );
                $filters = array();
                $_POST = $validator -> filter($_POST, $filters);
                $validated = $validator -> validate($_POST, $rules);
                if ($validated === TRUE) {
                ...
                if ($_POST['tipo'] == 4) {
                    $rules = array('tautor' => 'required|alpha', 'ttitulo' => 'required|alpha', 'nacionalidad' => 'required|alpha', 'ttipo' => 'required', 'tfechapublicacion' => 'required', );
                    $filters = array();
                    $_POST = $validator -> filter($_POST, $filters);
                    $validated = $validator -> validate($_POST, $rules);
                    if ($validated === TRUE) {
...
$app -> post('/formulario-registro-post/', function() use ($app) {
    $validator = new GUMP();
    $_POST = $validator -> sanitize($_POST);
    $rules = array('nombre' => 'required|alpha', 'ap' => 'alpha', 'am' => 'alpha', 'email' => 'required|valid_email', 'usuario' => 'required|alpha', 'pass' => 'required|alpha', 'confirmacion' => 'required|alpha', );
    $filters = array('pass' => 'md5');
    $_POST = $validator -> filter($_POST, $filters);
    $validated = $validator -> validate($_POST, $rules);
    if ($validated === TRUE) {
        if ($_POST['pass'] == $_POST['confirmacion']) {
            ...
            $flash = array("title" => "OK", "msg" => "Los Campos de Confimacion y contraseña deben ser identicos.", "type" => "error", "fade" => 1);
            $app -> flash("flash", $flash);
            $app -> flashKeep();
            $app -> redirect($app -> urlFor('registro-aspirantes'));
        }
...
$app -> post('/nuevo-tesista/', function() use ($app) {
    ...
    $validator = new GUMP();
    $_POST = $validator -> sanitize($_POST);
    $rules = array('lineaInv' => 'required|alpha', 'tesista' => 'required|alpha', 'nombreTesista' => 'required|alpha', 'ingreso' => 'required', );
...
$app -> post('/nuevo-info-contacto/', function() use ($app) {
    ...
    $validator = new GUMP();
    $_POST = $validator -> sanitize($_POST);
    $rules = array('email' => 'required', 'enterado' => 'required', 'fijo' => 'required', );
    $filters = array('email' => 'trim', 'enterado' => 'trim', 'mantener' => 'trim', 'movil' => 'trim', 'fijo' => 'trim', );
    $_POST = $validator -> filter($_POST, $filters);
    $validated = $validator -> validate($_POST, $rules);
    if ($validated === true) {
        if (isset($_POST['id'])) {
...
$app -> get('/admin/catalogos/areas-interes/', function() use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"), array("name" => "Catálogos", "alias" => "admin-catalogos"), array("name" => "Areas de Interes", "alias" => "CatAreaInteres"));
...
$app -> get('/admin/catalogos/idiomas/', function() use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"), array("name" => "Catálogos", "alias" => "admin-catalogos"), array("name" => "Idiomas", "alias" => "CatIdiomas"));
    $data['idiomas'] = Idioma::all();
...
$app -> post('/nuevo-idioma/', function() use ($app) {
    ...
    if ($validated === true) {
        ...
        $idiomas -> save();
        $flash = array("title" => "OK", "msg" => "Idioma se agregó satisfactoriamente .", "type" => "success", "fade" => 1);
        $app -> flash("flash", $flash);
        $app -> flashKeep();
...
$app -> post('/actualiza-idioma/:id/', function($id) use ($app) {
    ...
    if ($validated === true) {
        ...
        $idioma -> save();
        $flash = array("title" => "OK", "msg" => "Datos del Idioma han sido actualizados correctamente.", "type" => "info", "fade" => 1);
        $app -> flash("flash", $flash);
        $app -> flashKeep();
        $app -> redirect($app -> urlFor('CatIdiomas'));
    } else {
...
$app -> get('/admin/catalogos/lenguajes/', function() use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"), array("name" => "Catálogos", "alias" => "admin-catalogos"), array("name" => "Lenguajes", "alias" => "CatLenguajes"));
    $data['lenguajes'] = Lenguaje::all();
...
$app -> get('/admin/catalogos/plataformas/', function() use ($app) {
    $data['breadcrumb'] = array( array("name" => "Panel de Control", "alias" => "admin"), array("name" => "Catálogos", "alias" => "admin-catalogos"), array("name" => "Plataformas", "alias" => "CatPlataformas"));
    $data['plataformas'] = Plataforma::all();
...
$app -> post('/nuevo-evento/', function() use ($app) {
    $validator = new GUMP();
    $_POST = $validator -> sanitize($_POST);
    $rules = array('nombre' => 'required|max_len,100|min_len,1', 'descripcion' => 'required|max_len,100|min_len,1', 'fecha_inicio' => 'required|max_len,100|min_len,1', 'fecha_fin' => 'required|max_len,100|min_len,1', 'prioridad' => 'required|max_len,100|min_len,1', 'hora_inicio' => 'required|max_len,10|min_len,1', 'hora_fin' => 'required|max_len,100|min_len,1', );
    $filters = array('nombre' => 'trim|sanitize_string', );
    $post = $_POST = $validator -> filter($_POST, $filters);
    $validated = $validator -> validate($_POST, $rules);
    if ($validated === TRUE) {
